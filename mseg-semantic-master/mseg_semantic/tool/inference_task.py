...
from mseg.utils.mask_utils_detectron import Visualizer
...
class InferenceTask:
    ...
    def render_single_img_pred(self, min_resolution: int = 1080) -> None:
        ...
        # # 创建相同大小的全为0的rgb_img1
        # rgb_img1 = np.zeros_like(rgb_img)
        # # 选择要复制的像素类别列表
        # desired_labels = [34, 35]
        # # 根据pred_label_img选择对应的坐标
        # indices = np.where(np.isin(pred_label_img, desired_labels))
        # # 将rgb_img中对应坐标的值复制到rgb_img1
        # rgb_img1[indices] = rgb_img[indices]
        # cv2.imwrite(output_gray_fpath, rgb_img1)

        # prediction = self.execute_on_img1(rgb_img)
        ...
        overlaid_img = frame_visualizer.overlay_instances(
            label_map=pred_label_img, id_to_class_name_map=self.id_to_class_name_map, alpha=1
        )

        imageio.imwrite(output_demo_fpath, overlaid_img)
        pred_label_img = np.expand_dims(pred_label_img, -1)
        # cv2.imwrite(output_gray_fpath, pred_label_img)
    ...
    def execute_on_img1(self, image: np.ndarray) -> np.ndarray:
        """
    ...
    def execute_on_dataloader(self, test_loader: torch.utils.data.dataloader.DataLoader) -> None:
        ...
        if self.args.save_folder == "default":
            self.args.save_folder = f"{self.args.input_file}/sam"
        ...
        for i, (input, _) in enumerate(test_loader):
            ...
            overlaid_img = frame_visualizer.overlay_instances(
                label_map=gray_img, id_to_class_name_map=self.id_to_class_name_map, alpha=1
            )
            output_demo_fpath = os.path.join(self.args.save_folder, image_name + ".png")
            imageio.imwrite(output_demo_fpath, overlaid_img)
