...
from mseg.utils.fb_colormap import random_color, fixed_color, point_color
...
class Visualizer:
    ...
    def rgb_instances(self, label_map):

        pred_color = np.zeros((label_map.size[1], label_map.size[0], 3), dtype=np.uint8)
        for label_idx in np.unique(label_map):
            color = fixed_color(rgb=True, maximum=255, idx=label_idx)
            pred_color[label_map==label_idx]=color
    ...
    def overlay_instances(self, label_map, id_to_class_name_map, alpha=0.5):
        ...
        for label_idx in np.unique(label_map):
            ...
            color = point_color(rgb=True, maximum=1, idx=label_idx)
    ...
    def draw_polygon(self, segment, color, edge_color=None, alpha=0.5):
        ...
        polygon = mpl.patches.Polygon(
            ...
            edgecolor=edge_color,
            # linewidth=max(self._default_font_size // 15 * self.output.scale, 1),
            linewidth=min(self._default_font_size // 15 * self.output.scale, 0),
        )
